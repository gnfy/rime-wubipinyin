...
打 zzg            → 简拼，同样出中国
打 zweixiao       → 微笑 😊
打 z              → 只按引导键：2026-09-13〔rq〕/ 09:15〔sj〕/ 星期六〔xq〕… 每个功能一条，兼做菜单
...
`z` 是所有辅助功能的总入口，候选框会显示 `〔拼音 · 时间〕` 提示。**只按一个 `z`** 就会列出今天日期、现在时间、星期、时间戳各一条，注释里写着触发词——不用记，看一眼就知道 z 能干什么；想要更多格式再补 `rq` / `sj`。五笔码里没有 z，所以永远不会误触发。
